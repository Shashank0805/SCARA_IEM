% Extracts:
%   - static mapping (steady speed vs voltage)
%   - per-sheet dynamic tau (time constant) using exponential fit
%   - builds effective transfer function (TF + SS form)
%   - validates TF against 5V and 6V data
...
%% Static mapping (linear regression using only 5V & 6V)
valid_idx = ismember({results.sheet},{'5V','6V'});
V_reg = V_list(valid_idx);
W_reg = W_list(valid_idx);

X = [V_reg ones(size(V_reg))];
theta = X\W_reg;
a = theta(1); b = theta(2);   % slope & intercept
yfit = X*theta;
SSres = sum((W_reg - yfit).^2);
SStot = sum((W_reg - mean(W_reg)).^2);
R2 = 1 - SSres/SStot;

fprintf('\n=== Static mapping (only 5V & 6V) ===\n');
fprintf('W_ss ≈ %.4f * V + %.4f  [rad/s], R^2=%.3f\n', a, b, R2);

figure('Name','Steady speed vs Voltage'); hold on; grid on;
scatter(V_list,W_list,80,'filled','DisplayName','All Data');
scatter(V_reg,W_reg,80,'r','filled','DisplayName','Used for regression');
V_line = linspace(min(V_reg),max(V_reg),100)';
y_line = a*V_line + b;
plot(V_line,y_line,'-k','LineWidth',1.5,'DisplayName','Linear fit (5V-6V)');
xlabel('Voltage (V)'); ylabel('Steady Speed (rad/s)');
title('Static mapping (steady speed vs supply voltage)');
legend;

%% Effective transfer function (using 5V & 6V tau)
Ksys = a;                       % slope from regression
tau_eff = mean(Tau_list(3:4));  % average tau from 5V and 6V
bias = b;                       % intercept

sys_tf = tf(Ksys,[tau_eff 1]);
disp('Identified Transfer Function (without bias):');
sys_tf
